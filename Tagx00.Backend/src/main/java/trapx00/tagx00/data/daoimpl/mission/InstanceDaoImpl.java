            ...
            for (int j = 0; j < instances1.size(); j++) {
                if (intances.get(i).getInstanceId() == instances1.get(j).getInstanceId()) {
                    if (temp.getMissionType().equals(MissionType.IMAGE)) {
                        Instance instanceDetailVo = (Instance) instances1.get(j);
                        Instance result=new Instance(instanceDetailVo.getWorkerUsername(), instanceDetailVo.getMissionInstanceState(),
